...
function ConversorMoedas() {
  ...
  const FIXER_URL = 'http://data.fixer.io/api/latest?access_key=8089202ca9c3b1f5ee9c0b257d9d84de'
  const [valor, setValor] = useState('1')
  const [moedaDe, setMoeadeDe] = useState('BRL')
  ...
  function handleMoedaDe(event) {
    setMoeadeDe(event.target.value)
  }
  ...
  function converter(event) {
    ...
    if (event.currentTarget.checkValidity() === true) {
      //se o formulário está correto, vou requisitar a API
  ...
  return (
    <div>
      ...
      <Alert variant="danger" show={exibirMsgErro}>
        Erro obtenado dados de conversão. Tente novamente.
      </Alert>
      <Jumbotron>
        <Form onSubmit={converter} noValidate validated={formValidado}>
          <Form.Row>
            ...
            <Col sm="1">
              <FontAwesomeIcon className="text-center" style={{ paddingTop: '5px' }} icon={faAngleDoubleRight} />
